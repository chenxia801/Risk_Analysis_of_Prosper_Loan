...
> In this project, an Exploratory Data Analysis (EDA) for the Prosper loan data set was conducted, in order to dig out some inspiring and valuable insights of the loan. Then machine learning algorithms were applied to predict the default risk. 
...
loan<-subset(loan, select=-c(CreditGrade, BorrowerAPR, LenderYield, EstimatedEffectiveYield, EstimatedLoss, EstimatedReturn,ProsperRating..numeric., ProsperRating..Alpha., ProsperScore, CurrentlyInGroup, GroupKey,CurrentCreditLines,TotalCreditLinespast7years, IncomeRange,IncomeVerifiable, PercentFunded,InvestmentFromFriendsCount,InvestmentFromFriendsAmount ))
...
```{r echo=FALSE,message=FALSE, warning=FALSE}
write.csv(loan, file='prosper_loan_clean.csv')
loan_clean<-subset(loan, select=-c(LoanStatus, BorrowerState, NewListingCat,                               fips.numeric,fips.character,
                             LoanCreationDate)) 
## convert employment status into dummy variables
loan_clean$EmploymentStatus<-as.character(loan_clean$EmploymentStatus)
loan_clean$EmploymentStatus[loan_clean$EmploymentStatus == "Not available"]
 
write.csv(loan_clean, file='loan_clean.csv')
```
...
- OOB is bottoming out at approximately 500 trees. So ntree=500 was chosen

###4.2.3 Final Random Forest Classification

- With the tunned parameters, the accuracy increased to 0.87. 
