...
#check the dimension of the loan after dropping columns 
dim(loan)
colSums(is.na(loan))
```
...
loan_predict$Employment_Status_Employed<-loan_predict$EmploymentStatus_Employed  + loan_predict$employed_fulltime 
names(loan_predict)
# Drop some columns after converting 
loan_predict<-subset(loan_predict, select=-c(IsBorrowerHomeowner,EmploymentStatus, EmploymentStatus_Employed, employed_fulltime ))
write.csv(loan_predict, file='loan_predict.csv')
...
loan_predict<-subset(loan_predict,c)
names(loan_predict)

loan_predict<-subset(loan_predict, select=-c(CreditScoreRangeUpper, TotalInquiries, OpenRevolvingAccounts,MonthlyLoanPayment,OpenRevolvingMonthlyPayment, EmploymentStatus_Other))
...
### 4.2.1 Run K-Fold cross validation 
```{r echo=FALSE,message=FALSE, warning=FALSE}
loan_pred<-loan_predict
...
glm_model <- train(DelinquentBorrowers~., data=loan_pred, trControl=train_control, method="glm")

# Summarise Logistic regression Results
summary(glm_model)

# Fit AdaBoosting model

sapply(loan_pred, class)
loan_pred$DelinquentBorrowers<-as.factor(loan_pred$DelinquentBorrowers)
boost_model <- train(DelinquentBorrowers~., data=loan_pred, trControl=train_control, method = 'adaboost')

# Summarise AdaBoosting Results
summary(boost_model)
...
### 4.2.2 Plot & compare ROC curves
```{r echo=FALSE,message=FALSE, warning=FALSE}
# L# List of predictions
preds_list <- list(glm_model, rf_model,boost_model)

# List of actual values (same for all)
m <- length(preds_list)
actuals_list <- rep(list(loan_predict$DelinquentBorrowers), m)
...
legend(x = "bottomright", 
       legend = c("Logistic regression", "Random Forest", "Adaboosting"),
       fill = 1:m)
```

### Apply model
the output throw out an error 'Coefficients: (2 not defined because of singularities)', linear regression functin lm() will result in "NA" as co-efficient for highly correlated variables.
